...
if(isset($_POST["blogSubmit"])) {
    ...
    if (move_uploaded_file($_FILES['blogImage']['tmp_name'], 'D:/Download/htdocs/vers1/Images/' . $blogImage)){
        $blogImagePath = 'Images/' . $blogImage;
    }else{
        echo "Failed to move uploaded image";
    };

    if (move_uploaded_file($_FILES['authorImage']['tmp_name'], 'D:/Download/htdocs/vers1/Images/' . $authorImage)){
