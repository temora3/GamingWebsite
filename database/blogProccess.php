...
if(isset($_POST["blogSubmit"])) {
    ...
    if (move_uploaded_file($_FILES['blogImage']['tmp_name'],dirname(__DIR__)."/Images/". $blogImage)){
        $blogImagePath = 'Images/' . $blogImage;
    }else{
        echo "Failed to move uploaded image";
    };

    if (move_uploaded_file($_FILES['authorImage']['tmp_name'],dirname(__DIR__)."/Images/" . $authorImage)){
